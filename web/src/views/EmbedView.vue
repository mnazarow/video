...
<template>
  <div class="embed">
    ...
    <template v-else-if="video">
      <VideoPlayer v-if="video.status === 'ready'" ref="player" :src="video.hlsUrl" :mp4="video.mp4Url" :poster="video.thumbnailUrl" :subtitles="video.subtitles" :chapters="video.chapters" :storyboard="video.storyboardUrl" :duration="video.duration" :start-at="startAt" :autoplay="autoplay" compact :allow-theater="false" :allow-mini="false" class="embed-player" @progress="onProgress" @timeupdate="onTime" @ready="send('ready', stateOf(0))" @play="send('play', stateOf())" @pause="send('pause', stateOf())" @ended="send('ended', { ...stateOf(video.duration), percent: 100 })" />
